...
public class QuickSortLog2 {
	...
	public static void main(String args[]) {
		...
		stringList.add(s5);
		stringList.add(s6);
		lessThanModified(s2,s3);
		String[] strArray =new String[stringList.size()]; 
		String[] array = stringList.toArray(strArray);
		...
		QuickSortLog2 ob = new QuickSortLog2();

		System.out.println("\n\n\nsorted array");
		printArray(arr);*/
	}
	public static boolean lessThanModified(String s1,String s2) {
		int compareStrings = compareStrings(s1,s2);
	...
	static int partition(String arr[], int low, int high) {
		...
		int i = (low - 1);
		for (int j = low; j < high; j++) {
			if (lessThanModified(arr[j], pivot)) {
				i++;
